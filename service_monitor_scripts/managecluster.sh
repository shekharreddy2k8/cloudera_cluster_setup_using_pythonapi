...
`echo $todo |grep -w 'start\|stop\|status\|restart' >/dev/null`
...
checkStatus()
{
  $MAS_SERVICE_MON_DIR/managecluster.py status $1
  return $?
}

stopService()
{
  $MAS_SERVICE_MON_DIR/managecluster.py stop $1
  return $?
}

startService()
{
  $MAS_SERVICE_MON_DIR/managecluster.py start $1
  return $?
}

restartService()
{
  $MAS_SERVICE_MON_DIR/managecluster.py restart $1
  return $?
}
